...
import { assertEvmAddress } from "@kiteagentos/core";
...
app.get("/health", (c) => c.json({ ok: true, service: "kiteagentos" }));
app.get("/modules", (c) => c.json({ modules }));
app.get("/agents", (c) => c.json({ agents: items, items }));
...
app.get("/agents/new", (c) => c.json({ route: "/agents/new", product: "KiteAgentOS", preview: true, modules }));
app.get("/approvals", (c) => c.json({ route: "/approvals", product: "KiteAgentOS", preview: true, modules }));
app.get("/runs", (c) => c.json({ route: "/runs", product: "KiteAgentOS", preview: true, modules }));
app.get("/settings", (c) => c.json({ route: "/settings", product: "KiteAgentOS", preview: true, modules }));
